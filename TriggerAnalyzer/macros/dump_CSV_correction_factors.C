...
void dump_CSV_correction_factors(TString inputFileName  = "file.root") {
  ...
  for( int iSys=0; iSys<numCSVSys; iSys++ ){
	...
	h_perjet_wgt_csv_Sys[iPt][iEta][iSys] = (TH1D*)histFile->Get(Form("h_perjet_wgt_csv_iPt%d_iEta%d_Sys%d",iPt,iEta,useSys));
	if( iEta<1 ) h_perjet_wgt_csv_hf_Sys[iPt][iEta][iSys] = (TH1D*)histFile->Get(Form("h_perjet_wgt_csv_hf_iPt%d_iEta%d_Sys%d",iPt,iEta,useSys));
	if( iEta<1 ) h_perjet_wgt_csv_cf_Sys[iPt][iEta][iSys] = (TH1D*)histFile->Get(Form("h_perjet_wgt_csv_cf_iPt%d_iEta%d_Sys%d",iPt,iEta,useSys));
	if( iPt<4  ) h_perjet_wgt_csv_lf_Sys[iPt][iEta][iSys] = (TH1D*)histFile->Get(Form("h_perjet_wgt_csv_lf_iPt%d_iEta%d_Sys%d",iPt,iEta,useSys));
      }
    }
  }
